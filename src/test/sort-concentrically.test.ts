...
const nonDeterministicCommands = ['sortConcentricallyShuffle'];
const extCommands: string[] = packageJson.contributes.commands.map(c => c.command.replace('sortConcentrically.', '')).filter(c => nonDeterministicCommands.indexOf(c) === -1);
        ...
        test(fixture, done => {
          ...
          commands.executeCommand('workbench.action.closeActiveEditor').then(() => {
            return window.showTextDocument(Uri.file(path.join(fixtureDir, `${fixture}_fixture`))).then(editor => {
              ...
              commands.executeCommand(`sortConcentrically.${extCommand}`).then(() => {
